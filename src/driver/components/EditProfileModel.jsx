...
const EditProfileModal = ({ profile, onClose, onUpdate }) => {
  const [name, setName] = useState(profile.driver_name);
  const [village, setVillage] = useState(profile.driver_village);
  const [file, setFile] = useState(null);
  const [preview, setPreview] = useState(
    profile.driver_image_path
      ? `${DRIVER_CLOUDINARY_URL}${profile.driver_image_path}`
      : null
  );
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      formData.append("driver_village", village);

      if (file) {
        formData.append("driver_photo", file);
      }
      
      const res = await fetch(`${API_URL}/api/driver/update`, {
        method: "PATCH",
        headers: {
